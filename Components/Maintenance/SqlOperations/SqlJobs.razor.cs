...
public partial class SqlJobs : ComponentBase
{
    ...
    protected List<SqlJobModel> Jobs { get; set; } = new();

    protected IEnumerable<SqlJobModel> FilteredRows =>
        string.IsNullOrWhiteSpace(SearchText)
            ? Jobs
            : Jobs.Where(MatchesSearch);
    ...
    {
        IsLoading = true;
        try { Jobs = (await SqlOperationsService.GetJobsAsync()).ToList(); }
        finally { IsLoading = false; }
    }

    protected bool MatchesSearch(SqlJobModel item)
    {
