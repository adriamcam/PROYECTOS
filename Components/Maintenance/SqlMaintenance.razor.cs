...
public partial class SqlMaintenance : ComponentBase
{
    ...
    protected async Task LoadDashboardAsync()
    {
        ...
        try
        {
            Dashboard = await SqlMaintenanceService.GetDashboardAsync(RetentionDays);
            Dashboard.LastExecution = LastExecution;
        }
        finally
        {
            IsExecuting = false;
    ...
    protected async Task ExecuteCleanupAsync(SqlMaintenanceTableSummaryModel table)
    {
        var confirmed = await JSRuntime.InvokeAsync<bool>(
            "confirm",
            $"Se eliminarán {table.EligibleToDelete:N0} registros de {table.TableName}. ¿Desea continuar?");
    ...
    protected static string GetIcon(string tableName)
    {
        return tableName switch
        {
            "AlertsManagement" => "🔔",
            "AzureAlertCloseQueue" => "✉️",
            "AlertasBackup" => "🗄️",
            "AlertUpdatesHistory" => "🕘",
            _ => "🧩"
        };
    ...
    private SqlMaintenanceRequestModel BuildRequest(string tableName)
    {
        return new SqlMaintenanceRequestModel
        {
            TableName = tableName,
