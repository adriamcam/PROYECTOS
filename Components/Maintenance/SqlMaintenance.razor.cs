...
public partial class SqlMaintenance : ComponentBase
{
    ...
    {
        var confirmed = await JSRuntime.InvokeAsync<bool>("confirm", $"Se eliminarán {table.EligibleToDelete:N0} registros de {table.TableName}. ¿Desea continuar?");
        if (!confirmed) return;
        IsExecuting = true;
        try
        {
    ...
    protected async Task UpdateStatisticsAsync()
    {
        IsExecuting = true;
        try { LastExecution = await SqlMaintenanceService.UpdateStatisticsAsync(BuildRequest("ALL")); await LoadDashboardAsync(); }
        finally { IsExecuting = false; }
    }
    protected async Task RebuildIndexesAsync()
    {
        var confirmed = await JSRuntime.InvokeAsync<bool>("confirm", "La reconstrucción de índices puede tomar varios minutos. ¿Desea continuar?");
        if (!confirmed) return;
        IsExecuting = true;
        try { LastExecution = await SqlMaintenanceService.RebuildIndexesAsync(BuildRequest("ALL")); await LoadDashboardAsync(); }
        finally { IsExecuting = false; }
    }
