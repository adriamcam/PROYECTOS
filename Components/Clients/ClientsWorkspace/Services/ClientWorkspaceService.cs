...
public sealed class ClientWorkspaceService : IClientWorkspaceService
{
    ...
    public ClientWorkspaceService(IConfiguration configuration)
    {
        _connectionString =
            configuration.GetConnectionString("ReportesDb")
        ...
        CancellationToken cancellationToken = default)
    {
        const string sql = """
            ...
            (
                SELECT
                    TRY_CONVERT(uniqueidentifier, CustomerTenantId)
                        AS TenantId,
                    PartnerTenant,
                    StatusFound,
                    ROW_NUMBER() OVER
